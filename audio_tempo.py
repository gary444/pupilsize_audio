...
#import library here so iview X can connect
from pyo import *
...
#    print sd1_target
    ...
    ### Patrick
    # Test 1 - change window color according to change in pup size
#    cur_win_color_g = 0
#    temp_win_color_g = 0
#    prev_win_color_g = 0
#    test_1_first_run = True
#    damp_color_factor = 1.45 # (cur_color - prev_color) / damp_color_factor # this will ease the effect of sudden major increases/decreases
#    color_change_factor = 1.06
#    measure_value_time = 2 # in seconds
#    frame_change_on = math.ceil (expInfo["frameRate"] * measure_value_time)
    ...
    #    if routine_helper.get_frame() == 0 or routine_helper.get_frame() % frame_change_on == 0:
    
        if routine_helper.get_frame() == 0:

            # Initialize components for Routine "bl_instructions"
#            feedback_stim = visual.TextStim(win=win, name='feedback_stim',
#                    text=u'Fetching color...  ' + str(cur_win_color_g),
#                    font='Arial',
#                    units='pix', pos=[0, 0], height=30, wrapWidth=None, ori=0, 
#                    color="white", colorSpace='rgb', opacity=1,
#                    depth=0.0);
#            feedback_stim.draw()
            ...
            cur_pup_mean = (cur_pup_mean + prev_pup_mean) / 1.66 # reconsider
            
            #a.setFreq(cur_pup_mean * 1000)
            
            
            bl_mean_dev = (bl_min_deviation + bl_max_deviation) / 2
            if bl_mean_dev > 0 and cur_pup_mean > 0:
                
                
                #color_change_factor = 1 # reconsider
                upper_dev_bound = bl_mean_dev * color_change_factor
                lower_dev_bound = bl_mean_dev / color_change_factor
                
                #print("\n")
                #print( str(lower_dev_bound) + " " +str(bl_mean_dev) + " " + str(upper_dev_bound))
                #print( str(cur_pup_mean))
                #print("\n")
                
                pup_state = "DOWN"
                real_val = 0
                if (cur_pup_mean < bl_mean_dev):
                    r = bl_mean_dev - lower_dev_bound
                    v = cur_pup_mean - lower_dev_bound
                    real_val = 0.5 * ( v / r )
                    if real_val <=0:
                        print("cur_pup_mean: " + str(cur_pup_mean) )
                        print("lower_dev_bound: " + str(lower_dev_bound) )
                else:
                    pup_state = "UP"
#                    r = upper_dev_bound - bl_mean_dev
#                    v = cur_pup_mean - bl_mean_dev
#                    real_val = 0.5 + ( 0.5 * v / r )
                
                # Important - stay in the RGB color range (so max between 0 and 1 here)
#                real_val = 1 if real_val > 1 else real_val
#                real_val = 0 if real_val < 0 else real_val
                
                
                
                # important
#                prev_win_color_g = cur_win_color_g
                
                # Important, this insures we have values between -1 and 1 for the RGB color range of psychopy
#                cur_win_color_g = -1 +  2 * real_val
                
#                print(str(pup_state) + " to color " + str(cur_win_color_g) )
                
                #print("\n")
                #print("cur_win_color_g: " + str(cur_win_color_g))

#                if cur_win_color_g >= 1: cur_win_color_g = 1
#                if cur_win_color_g < -1: cur_win_color_g = -1
        
        ##########
        # On every frame
#        else:
#        #if test_1_first_run == False: # skip first time because we need one "prev" reading
#            
#            # absolute change in color
#            color_change_total = cur_win_color_g - prev_win_color_g
#            
#            
#            # more damping of changes
#            if color_change_total >= 0.4:
#                color_change_total = 0.4
#                #color_change_total = color_change_total / damp_color_factor
#                
#            if color_change_total <= -0.4:
#                color_change_total = -0.4
#                #color_change_total = color_change_total / damp_color_factor
#            
#            # color change for every frame until we sample the new color values
#            # the -1 here is because we want to reach the goal of setting the color in `frame_change_on-1` frames, because we won't be in this loop on each `frame_change_on` frames
#            color_change_frame = color_change_total / (frame_change_on-1)
#            
#            if color_change_total != 0:
#                
#                
#                # update the amount of color to change for next frame
#                temp_win_color_g = temp_win_color_g + color_change_frame
#                temp_win_color_g = -1 if temp_win_color_g < -1 else temp_win_color_g
#                temp_win_color_g = 1 if temp_win_color_g >1 else temp_win_color_g
                
                #print( str(temp_win_color_g) + "  " + str(color_change_total) )
                
                #print( str(temp_win_color_g) + " / " + str(color_change_total))
                #win.color = [-1, temp_win_color_g, -1]
                
                
        
        
        #if test_1_first_run: test_1_first_run = False
            
        # Save previous eye pupil mean
        prev_pup_mean = cur_pup_mean
