...
for thisTrial in trials:
    ...
    #parameters for pupil size values and musical note range
    SD_steps = 7
    targetValue = sd2_target;
    baseline = bl_mean;
    step_width = (targetValue - baseline) / SD_steps
   
   #function to convert pupil size difference into musical note difference
    ...
    freq = noteToFreq(targetTone - 24) #starting freq only
    env = Adsr(attack=.01, decay=.2, sustain=.5, release=.5, dur=1.5, mul=.5)
    #check this with headphones - does it send simulatneous signals to both ears?
    sig1 = RCOsc(freq, sharp=0.2, mul=env)
    sig2 = RCOsc(noteToFreq(targetTone), sharp=0.0, mul=env*0.5)
    #ensure equal level in both ears
    ...
    while routine_helper.get_status() and routineTimer.getTime() > 0:
        ...
        if lmarker == 0:
            ...
            if recent_avg >= targetValue:
                 #trigger success noise - after x time above threshold
                if holding:
                    holdCounter+=1
                    print(holdCounter)
